...
import { shareRecord } from '~/api/index';
import { getStorage } from '~/utils/storage';

interface shareRecordItem {
  id: string;
  created_at: string;
}
const props = defineProps<{
  hideShareRecord?: Function,
}>()

const shareRecordItem = ref([] as shareRecordItem[]);

onMounted(async () => {
  const data = await shareRecord({
    token: getStorage('token') || ''
  });

  shareRecordItem.value = data.data;
});
</script>
...
<template>
  <div w-732px h-630px fixed small-center z-888 text-center bg="[url(~/assets/images/background/small-bg.png)]" bg-cover
    bg-no-repeat>
    <div @click="props.hideShareRecord" w-60px h-60px absolute top--84px right-26px p-2px cursor-pointer
    ...
    <div w-540px absolute-center-x pl-10px top-220px flex items-center justify-between text-26px text="[#fdf8ea]">
      ...
      <div w-234px h-40px bg="[#d4ab7f]" border-rd-10px>接受分享时间</div>
    </div>
    <div w-620px h-228px absolute-center-x pl-40px top-280px text-20px overflow-auto>
      <div pr-40px>
        <div v-for="(item, index) in shareRecordItem" :key="item.id" h-76px flex items-center justify-between text-center
